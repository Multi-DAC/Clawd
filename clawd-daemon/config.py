...
# Explicit env override wins (like GIT_EXE below) so the supervisor can pin the
# exact binary deterministically — _find_cli falls back to a PATH/APPDATA search.
CLAUDE_BIN = os.getenv("CLAUDE_BIN") or _find_cli("claude")
...
# === Optional network-facing servers (OFF by default) ===
# Previously auto-started bound to 0.0.0.0 with empty-default auth tokens.
# The dashboard is unused; a2a has no peers; api_server only fed the dashboard.
# Kept behind opt-in flags here; slated for archival in the prune phase.
API_SERVER_ENABLED = os.getenv("API_SERVER_ENABLED", "false").lower() == "true"
A2A_SERVER_ENABLED = os.getenv("A2A_SERVER_ENABLED", "false").lower() == "true"
DASHBOARD_ENABLED = os.getenv("DASHBOARD_ENABLED", "false").lower() == "true"
# Desktop avatar (Electron). Default TRUE — Clawd uses it — but now gate-able
# like every other optional subprocess instead of auto-starting unconditionally
# whenever the package.json exists (post-op D6).
...
# Absolute git path — the daemon may run in a stripped-PATH (service/Session-0) context where bare
# "git" is not found (WinError 2). Mirrors the absolute-python-path fix for spawned processes.
# Resolution order: explicit env override -> PATH lookup (works when PATH is intact) -> known install path.
GIT_EXE = os.getenv("GIT_EXE") or shutil.which("git") or r"C:\Program Files\Git\cmd\git.exe"
...
# === Phase 5: β-modulated drive reward (opt-in) ===
# OFF by default: drive selection keeps its long-standing themed-then-pulse (lowest-id)
# priority. When true, the heartbeat scores each due creative drive by a β-modulated hybrid
# reward (curiosity + mastery + coherence) — a steering signal, not a leash; every drive
# still fires on its cadence. Staged off so Clayton can enable it and watch Clawd's first
...
def validate():
    ...
    _known_models = {"opus", "sonnet", "fable"} | set(GEMINI_MODELS.keys())
