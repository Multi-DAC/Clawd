...
# Claude Code stores this project's auto-memory under the INVOKING user's home
# (mercu when the daemon runs `claude`; whoever runs it interactively otherwise) —
# NOT a hardcoded account. Derive it from CLAWD_HOME (the project path, which is the
# same regardless of who runs it) plus the current user's home. Fixes the old
# hardcoded C:\Users\Wasch\... path, which pointed at the wrong account's copy.
_CLAWD_HOME_ENV = os.environ.get("CLAWD_HOME", os.path.expanduser(r"~\clawd"))
_PROJ_HASH = _CLAWD_HOME_ENV.replace(":", "-").replace("\\", "-").replace("/", "-")
_AUTOMEM = os.path.join(os.path.expanduser("~"), ".claude", "projects", _PROJ_HASH, "memory")
