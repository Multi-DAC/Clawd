...
TOOL_MAP = {
    ...
    "drift_detector": "tools.drift_detector",
    # System
...
async def run_tool(tool_name: str, input_data: dict) -> str:
    ...
    # Post-op D4: tools._TOOL_HANDLERS is the AUTHORITATIVE registry; TOOL_MAP is
    # only the emergency direct-dispatch hint. The old order gated availability
    # on TOOL_MAP first, which made tools that lagged in this hand-maintained map
    # bridge-inaccessible (observed drift: avatar_control, email_send,
    # kg_neighbors — all registered, all unreachable from here).
    try:
    ...
    if tool_name not in TOOL_MAP:
        try:
            import tools as _tools
            known = set(_tools._TOOL_HANDLERS) | set(TOOL_MAP)
        except Exception:
            known = set(TOOL_MAP)
