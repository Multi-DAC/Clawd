...
# Post-op A3: every query used to re-parse (and every mutation rewrite) the whole
# growing JSON on the event loop. mtime-keyed cache makes repeat reads free; the
# async tool handler below runs in a worker thread so the remaining IO is off-loop.
_graph_cache: dict | None = None
_graph_cache_mtime = 0.0


def _load_graph() -> dict:
    """Load knowledge graph from disk (mtime-cached)."""
...
async def _knowledge_graph(input_data: dict) -> str:
    """Async shim — body does file IO + full-graph scans; runs off-loop (A3)."""
    return await asyncio.to_thread(_knowledge_graph_sync, input_data)


def _knowledge_graph_sync(input_data: dict) -> str:
