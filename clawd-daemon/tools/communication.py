...
async def _tts_edge(text: str, voice: str, audio_path: Path) -> Path | None:
    ...
    tts_script = f"""
# Norton MITMs every HTTPS on this body; certifi/default ctx can't see its root, so
# edge-tts's wss:// to speech.platform.bing.com fails CERT_VERIFY and we silently drop
# to gTTS (not-Ryan). truststore uses the OS trust store (which DOES have Norton's root).
# MUST run before `import aiohttp` — aiohttp builds its verified SSL context at import time.
try:
