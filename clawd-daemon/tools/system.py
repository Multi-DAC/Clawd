...
TOOL_DEFINITIONS = [
    ...
    {
        "name": "switch_model",
        "description": "Switch active model. Available: opus (Claude Opus 4.8), sonnet (Claude Sonnet 4.6), fable (Claude Fable 5, most capable), gemini (Gemini 2.5 Pro), gemini-pro (Gemini 3.1 Pro Preview).",
        "input_schema": {
            "type": "object",
            "properties": {
                "model": {
                    "type": "string",
                    "enum": ["opus", "sonnet", "fable", "gemini", "gemini-pro"],
    ...
    {
        "name": "consult",
        "description": "Consult a sub-agent model for a specific task. Isolated call — no identity, no conversation history. opus/sonnet/fable: Claude Code (fable most capable, handles own tools). gemini/gemini-pro: Gemini CLI (large context, handles own tools).",
        "input_schema": {
            "type": "object",
            "properties": {
                "model": {
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "subtasks": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "model": {"type": "string", "enum": ["opus", "sonnet", "fable", "gemini", "gemini-pro"], "description": "Model to use. opus/sonnet for complex tasks, gemini for large context."},
                            "depends_on": {
                                "type": "array",
                                "items": {"type": "string"},
                ...
                "done_when": {
                    "type": "string",
                    "description": "Explicit completion criterion ('done means X'). If set, the plan is only reported completed after a verifier agent confirms the aggregate output satisfies it — otherwise it is marked failed with feedback. Use this for real tasks that must finish end-to-end (not open-ended exploration)."
                },
                "background": {
                    "type": "boolean",
                    "description": "If true, spawn as background task and return immediately with a task ID. Use check_background_task to poll results."
...
class ExecutionEngine:
    ...
    async def execute(self, plan_id: str, task_desc: str, subtasks: list[dict],
                      replan_enabled: bool = False, resume: bool = False,
                      done_when: str = None) -> str:
        """Execute a DAG plan with SQLite persistence.

        done_when: optional explicit completion criterion. If given, the plan is only
        reported 'completed' after a verifier agent confirms the aggregate output
        satisfies it (Phase 2 task done-gate) — the cure for tasks that look finished
        but aren't."""
        ...
        # Task done-gate (Phase 2): a plan is only 'completed' if a verifier confirms the
        # aggregate output satisfies the explicit done_when criterion — the cure for
        # 'looks finished but isn't'. Only runs when the caller declares done_when.
        gate_note = ""
        if done_when and final_status == "completed" and nodes:
            combined = "\n\n".join(f"[{n['id']}] {n.get('result','')[:1200]}" for n in nodes)
            passed, reason = await self._verify_done(task_desc, combined, done_when)
            if passed:
                gate_note = f"\n**DONE-GATE: satisfied** — {reason}"
            else:
                final_status = "failed"
                gate_note = (f"\n**DONE-GATE: NOT SATISFIED** — {reason}\n"
                             f"The task is NOT finished; marked failed so it isn't mistaken for done. Continue the work.")
    ...
    async def _verify_done(self, task_desc: str, combined_output: str, done_when: str):
        """Two-agent done-gate: an independent verifier judges whether the aggregate output
        satisfies the explicit completion criteria. Returns (passed: bool, reason: str).

        Fail-OPEN on verifier error: a verifier outage must not permanently block a task
        that may well be done — it logs and accepts, rather than wedging the work."""
        gate_prompt = (
            "You are a STRICT completion verifier. Decide whether the WORK OUTPUT fully "
            "satisfies the DONE-WHEN criteria for the TASK. Be skeptical of plausible-but-"
            "incomplete work. Reply on the FIRST line with exactly PASS or FAIL, then one "
            "short line of reason.\n\n"
            f"TASK:\n{task_desc[:1500]}\n\n"
            f"DONE-WHEN (completion criteria):\n{done_when[:800]}\n\n"
            f"WORK OUTPUT:\n{combined_output[:4000]}"
        )
        try:
            verdict = await self.router.consult_model(
                "gemini", gate_prompt, interrupt_event=self.interrupt_event)
            lines = [ln for ln in str(verdict).strip().splitlines() if ln.strip()]
            first = (lines[0] if lines else "FAIL").upper()
            passed = first.startswith("PASS")
            return passed, str(verdict).strip()[:300]
        except Exception as e:
            logger.warning(f"done-gate verifier failed ({e}); accepting output without gate")
            return True, f"[gate skipped: verifier error {e}]"
...
async def _plan_and_execute_inner(input_data: dict) -> str:
    ...
    done_when = input_data.get("done_when")  # explicit completion criterion -> done-gate
