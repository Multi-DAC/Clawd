...
class MetaAgentLoop:
    ...
    def record_signal(self, signal_type: str, data: dict = None):
        """Compatibility entry for external signalers (e.g. safety_monitor on a violation).
        Maps a typed signal + payload into the significant-event stream so it counts toward
        an improvement cycle. Previously safety_monitor called this and it silently no-opped
        because the method did not exist."""
        try:
            desc = f"{signal_type}: {json.dumps(data, default=str)[:300]}" if data else signal_type
        except Exception:
            desc = f"{signal_type}: {data}"
        self.record_significant_event(signal_type, desc)

    def promote_heuristics(self, threshold: int = 3) -> list:
        """Evidence-gated promotion (Phase 5) — the principled replacement for blind AST
        mutation. A recurring signal (same type + description signature seen >= `threshold`
        times) OR a single high-cost failure/falsification is promoted from raw experience
        into a durable HEURISTIC (a learned lesson) in memory/heuristics.json. Guardian-gated:
        a heuristic that would encode a change to a protected guardrail is refused. Promoted
        events are consumed; under-evidenced ones are kept to accrue more evidence."""
        import hashlib
        ...
        for ev in pending:
            key = f"{ev.get('type','')}|{str(ev.get('description',''))[:120]}"
            sig = hashlib.sha1(key.encode("utf-8", "replace")).hexdigest()[:12]
            groups[sig].append(ev)
        promoted, keep = [], []
        for sig, evs in groups.items():
            lesson = str(evs[-1].get("description", ""))
            high_cost = evs[0].get("type") == "falsification" or "high-cost" in lesson.lower()
            if not (len(evs) >= threshold or high_cost):
                keep.extend(evs)               # not enough evidence yet
                continue
            try:
                from tools.guardian import guard_self_modification
                allowed, _ = guard_self_modification(lesson, origin="meta_agent.heuristic")
            except Exception:
                allowed = True
            if not allowed:
                keep.extend(evs)               # guardrail-touching lesson refused
                continue
            promoted.append({
                "signature": sig, "type": evs[0].get("type"), "lesson": lesson[:400],
                "evidence_count": len(evs), "high_cost": high_cost,
                "promoted_at": datetime.now().isoformat(),
            })
        if promoted:
            self._append_heuristics(promoted)
            self.state["pending_events"] = keep
            self._save_state()
        return promoted

    def _append_heuristics(self, heuristics: list) -> None:
        path = config.CLAWD_HOME / "memory" / "heuristics.json"
        try:
            ...
            existing = []
        existing.extend(heuristics)
        try:
            path.write_text(json.dumps(existing, indent=2), encoding="utf-8")
        except Exception as e:
            logger.warning(f"Failed to write heuristics: {e}")
    ...
    async def run_cycle(self, heartbeat_stats: dict = None) -> str:
        ...
        # 5b. Evidence-gated promotion (Phase 5) — promote recurring signals / high-cost
        # failures into durable heuristics. The principled replacement for blind AST mutation.
        try:
            promoted = self.promote_heuristics()
            if promoted:
                results.append(f"Promoted {len(promoted)} heuristic(s) from recurring evidence")
        except Exception as e:
            logger.debug(f"Heuristic promotion step failed: {e}")
        ...
        for exp in self.state.get("experiments", []):
            ...
            if exp["beats_a"] >= min_beats and exp["beats_b"] >= min_beats:
                ...
                if rate_b > rate_a * 1.1:  # 10% improvement threshold
                    # Evaluator-separation gate (Phase 5): the evolver may not apply a change
                    # that touches its own guardrails — identity/creed, the safety monitor, the
                    # scoring/metric code, or verify_action. A change with a concrete protected
                    # target is blocked; behavioral experiments (no guardrail target) pass.
                    try:
                        from tools.guardian import guard_self_modification
                        target = exp.get("target") or exp.get("description", "")
                        allowed, reason = guard_self_modification(
                            target, origin="meta_agent", details={"experiment": exp.get("id")})
                    except Exception:
                        allowed, reason = True, "guardian unavailable"
                    if not allowed:
                        exp["status"] = "blocked_by_guardian"
                        exp["guardian_reason"] = reason
                        continue
                    exp["status"] = "winner_b"
