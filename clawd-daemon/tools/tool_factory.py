...
async def _create_tool(input_data: dict) -> str:
    ...
    # Post-op D3 (guardian parity): runtime tool creation IS self-modification —
    # route it through the same gate as meta_agent's apply path. Confinement to
    # CLAWD_HOME/tools/custom already limits blast radius; this adds the audit
    # trail and blocks protected-set collisions.
    from tools.guardian import guard_self_modification
    allowed, reason = guard_self_modification(
        f"tools/custom/{name}.json", origin="tool_factory",
        details={"description": (description or "")[:200]})
    if not allowed:
