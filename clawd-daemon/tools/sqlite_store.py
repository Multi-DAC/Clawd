...
async def _migrate_kg_dedup(db) -> str:
    """One-time (idempotent) KG edge de-bloat + uniqueness guard.

    The JSON->SQLite edge migration used a plain INSERT that re-ran every boot, so
    kg_edges grew ~45x (measured: 1,127,149 rows / 207 MB for 25,109 distinct edges) and
    neighbor reads returned mostly duplicate garbage — 'the KG never worked'. This removes
    duplicate edges (keeping the lowest id per from/to/relation/valid_from/valid_to) and
    adds a UNIQUE index so it can NEVER re-bloat (the migration insert is now
    INSERT OR IGNORE, which relies on this index). No-op once the table is clean.
    """
    try:
        total = (await (await db.execute("SELECT COUNT(*) FROM kg_edges")).fetchone())[0]
        distinct = (await (await db.execute(
            "SELECT COUNT(*) FROM (SELECT DISTINCT from_entity,to_entity,relation,"
            "valid_from,IFNULL(valid_to,'') FROM kg_edges)"
        )).fetchone())[0]
        removed = 0
        if total > distinct:
            logger.warning(f"KG edge de-bloat: {total} rows -> {distinct} distinct; removing {total - distinct} duplicates")
            await db.execute("""
                DELETE FROM kg_edges WHERE id NOT IN (
                    SELECT MIN(id) FROM kg_edges
                    GROUP BY from_entity, to_entity, relation, valid_from, IFNULL(valid_to,'')
                )
            """)
            await db.commit()
            removed = total - distinct
        # Uniqueness guard (idempotent). Expression index normalizes NULL valid_to so
        # active edges (valid_to IS NULL) dedup too — plain UNIQUE treats NULLs as distinct.
        await db.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_kg_edges_unique "
            "ON kg_edges(from_entity, to_entity, relation, valid_from, IFNULL(valid_to,''))"
        )
        await db.commit()
        if removed:
            try:
                ...
                logger.warning(f"KG de-bloat VACUUM skipped (non-fatal): {ve}")
        return f"KG edges de-bloated: removed {removed} duplicates ({distinct} distinct kept)" if removed else ""
    except Exception as e:
        logger.error(f"KG edge de-bloat failed (non-fatal): {e}")
        return ""
...
async def migrate_json_to_sqlite():
    ...
    # 5. Knowledge graph migration RETIRED (post-op D2, 2026-07-01).
    # Locked decision: ONE working KG store. Every live read/write path
    # (knowledge_graph.py, kg_neighbors.py, memory_search graph+fusion channels,
    # consolidation, memory_agent) uses knowledge_graph.json; the SQLite kg_*
    # tables were a boot-refreshed dead mirror with zero readers — the surface
    # that once bloated 43x to 1.1M rows, and a standing risk of the two stores
    # being re-wired to disagree again. The tables stay on disk untouched (data
    # is sacred; pre-debloat backup exists) but are no longer written.
    results.append("KG migration retired — knowledge_graph.json is the single store")
