...
# Cross-encoder reranker — lazy-loaded. Post-op P0-4: load failure is a timed
# backoff, not a permanent per-process disable (a single transient failure used
# to kill reranking for the process's whole life, invisibly).
_cross_encoder = None
_cross_encoder_retry_after = 0.0
_DEGRADE_RETRY_S = 1800  # retry a failed model/index load every 30 min


def _mark_recall_degraded(reason: str) -> None:
    """Post-op P0-4: recall degradation must be LOUD and must retry.

    Sets a 30-min retry backoff (was: permanent flag) and drops a marker file
    the monitor layer watches. The retrieval canary tests a DIFFERENT process,
    so without this marker a degraded serving process looks green from outside.
    """
    global _index_retry_after
    _index_retry_after = time.monotonic() + _DEGRADE_RETRY_S
    try:
        import os
        import sys
        marker = config.CLAWD_HOME / "memory" / "recall_degraded.json"
        marker.write_text(json.dumps({
            "ts": datetime.now().isoformat(),
            "pid": os.getpid(),
            "argv0": sys.argv[0] if sys.argv else "",
            "reason": reason,
            "retry_in_s": _DEGRADE_RETRY_S,
        }, indent=2), encoding="utf-8")
    except Exception:
        pass


def _clear_recall_degraded() -> None:
    try:
        marker = config.CLAWD_HOME / "memory" / "recall_degraded.json"
        if marker.exists():
            marker.unlink()
...
def _get_cross_encoder():
    ...
    """
    global _cross_encoder, _cross_encoder_retry_after
    if _cross_encoder is not None:
        return _cross_encoder
    if time.monotonic() < _cross_encoder_retry_after:
        return None
    try:
        import torch
        ...
        logger.warning(f"Cross-encoder unavailable (RRF-only mode, retry in {_DEGRADE_RETRY_S}s): {e}")
        _cross_encoder_retry_after = time.monotonic() + _DEGRADE_RETRY_S
        return None
...
# Lazy per-process semantic-index init (Day 150 fix). The daemon's async boot
# runs init_embedding_index() in the HEARTBEAT process only; the MCP-server
# process that actually serves memory_search never ran it, so its _embedding_index
# stayed None and vector/hybrid search silently degraded to keyword-only even
# though the on-disk index was healthy. This restores the sense in whatever
# process first needs it, without ever triggering an expensive cold rebuild.
_index_init_lock = asyncio.Lock()
_index_retry_after = 0.0  # P0-4: timed backoff, not a permanent disable

# Path components whose files pollute memory recall — vendored deps, archived/dormant
# skills, and skill code-resources — excluded from the keyword scan (Day 150). Memory
# lives in memory/, projects/, identity/ and never under these; skill bundles do.
_INDEX_SKIP_PARTS = frozenset({"node_modules", "_archive", "__pycache__", ".git", "resources"})


async def _ensure_embedding_index():
    """Return the semantic index, lazily loading it if this process skipped boot init.

    Loads ONLY when a prebuilt index exists on disk (a tool call must never kick
    off a full cold rebuild). Concurrency-safe and idempotent; on failure it marks
    the attempt so we don't re-pay the model-load cost on every query.
    """
    global _embedding_index, _index_retry_after
    ...
    async with _index_init_lock:
        ...
        if time.monotonic() < _index_retry_after:
            return None
        try:
            load_budget = getattr(config, "MEMORY_INDEX_LOAD_TIMEOUT", 60)
            # Day-151 reboot-wedge fix: the IMPORT itself (tools.embeddings → numpy →
            # C-extension DLL load) used to run here, synchronously ON the event loop.
            # On the first post-reboot recall, the AV on-access scan held that DLL load
            # for minutes; the loop froze, so the wait_for guards below (and every
            # dispatch timeout upstream) could never fire. Import in a worker thread
            # under the same budget — a blocked import now degrades this process to
            # keyword-only instead of wedging every tool call in the process.
            def _import_embeddings_mod():
                from tools import embeddings
                return embeddings
            try:
                emb = await asyncio.wait_for(
                    asyncio.to_thread(_import_embeddings_mod), timeout=load_budget
                )
            except asyncio.TimeoutError:
                _mark_recall_degraded(f"tools.embeddings import exceeded {load_budget}s (blocked DLL load?)")
                logger.warning(
                    f"tools.embeddings import exceeded {load_budget}s (blocked DLL load?) — "
                    f"keyword recall for this process; retry in {_DEGRADE_RETRY_S}s."
                )
                return None
            if not (emb.EMBEDDINGS_FILE.exists() and emb.METADATA_FILE.exists()):
                _index_retry_after = time.monotonic() + _DEGRADE_RETRY_S
                logger.info("Semantic index not on disk — keyword-only; recheck in 30 min.")
                return None
            idx = emb.EmbeddingIndex()
            # Day-151 wedge fix: LOAD the prebuilt index, never build() on a tool call.
            # build() hash-walks the corpus + re-embeds deltas + loads the model (29s warm,
            # minutes under CPU contention) — that was the recall freeze, and the earlier
            # wait_for(build) guard couldn't help because the block was really downstream on
            # the event loop. load_for_search() only reads the npz/json + loads the query
            # encoder, in a thread. Still time-bounded so a cold model load can't wedge the
            # caller; boot init keeps the on-disk index fresh.
            try:
                loaded = await asyncio.wait_for(idx.load_for_search(), timeout=load_budget)
            except asyncio.TimeoutError:
                _mark_recall_degraded(f"index load exceeded {load_budget}s")
                logger.warning(
                    f"Semantic index load exceeded {load_budget}s — keyword recall for this "
                    f"process; on-disk index unchanged, retry in {_DEGRADE_RETRY_S}s."
                )
                return None
            if not loaded:
                _mark_recall_degraded("load_for_search returned no usable index")
                logger.info("Semantic index load returned no usable index — keyword-only; will retry.")
                return None
            set_embedding_index(idx)
            _clear_recall_degraded()
            logger.info(f"Lazily loaded semantic index in this process: {idx.chunk_count} chunks")
            return _embedding_index
        except ImportError:
            _mark_recall_degraded("sentence-transformers not importable")
            logger.warning("sentence-transformers not available — semantic search disabled; will retry.")
            return None
        except Exception as e:
            _mark_recall_degraded(f"lazy load failed: {e}")
            logger.warning(f"Lazy semantic-index load failed (retry in {_DEGRADE_RETRY_S}s): {e}")
            return None
...
async def _memory_search(input_data: dict) -> str:
    ...
    if strategy == "auto":
        # Hybrid RRF when the semantic index is available (lazy-load it if this
        # process skipped boot init), else keyword.
        await _ensure_embedding_index()
        if _embedding_index is not None:
            try:
        ...
        idx = await _ensure_embedding_index()
        if idx is None:
        ...
        return await asyncio.to_thread(_graph_search, query, top_k, include_metadata)
...
async def _kg_entity_search(query: str, max_results: int = 20) -> list:
    """Entity-fusion retrieval channel (Phase 3): query -> matching KG entities -> their
    active neighbor edges, returned as (source, content, score) tuples for RRF fusion.

    Only safe now that the KG reads back real neighbors (the de-bloat + uniqueness guard).
    Honors bi-temporal invalidation (skips edges with a non-null valid_to). Best-effort:
    never raises into the caller — a KG hiccup must not break memory_search."""
    try:
        import re as _re
        loop = asyncio.get_event_loop()
        from tools.kg_neighbors import _load_graph
        graph = await loop.run_in_executor(None, _load_graph)
        entities = graph.get("entities", {}) or {}
        edges = graph.get("edges", []) or []
        if not entities or not edges:
            return []
        terms = [t for t in _re.findall(r"[a-z0-9_]+", query.lower()) if len(t) >= 3]
        if not terms:
            return []
        matched = set()
        for eid, e in entities.items():
            name = str(e.get("name", eid)).lower()
            if any(t in name or t in str(eid).lower() for t in terms):
                matched.add(eid)
                if len(matched) >= 25:
                    break
        if not matched:
        ...
        for edge in edges:
            ...
            if vt and not isinstance(vt, dict):
                continue  # bi-temporal: skip invalidated edges
            frm, to, rel = edge.get("from", ""), edge.get("to", ""), edge.get("relation", "")
            if frm in matched or to in matched:
                fn = entities.get(frm, {}).get("name", frm)
                tn = entities.get(to, {}).get("name", to)
                content = f"[KG] {fn} --{rel}--> {tn}"
                if content in seen:
                    continue
                seen.add(content)
                results.append((f"kg:{frm}->{to}:{rel}", content, 1.0))
                if len(results) >= max_results:
                    break
        return results
    except Exception as e:
        logger.debug(f"KG entity-fusion channel skipped: {e}")
        return []


async def _hybrid_search(query: str, top_k: int = 10, include_metadata: bool = False) -> str:
    """Hybrid search: vector + keyword + items + FTS5 + KG entity-fusion, merged with
    Reciprocal Rank Fusion, then cross-encoder reranked for precision."""
    # Retrieve more candidates for reranking (top-30 each instead of 15).
    # Day-151: every synchronous model/IO call below runs in a thread so this coroutine
    # keeps yielding — otherwise it blocks the loop and no timeout upstream can fire.
    loop = asyncio.get_event_loop()
    vector_results = await loop.run_in_executor(None, lambda: _embedding_index.search(query, top_k=30))
    ...
    try:
        from tools.memory_items import search_items_raw
        item_results = await loop.run_in_executor(None, lambda: search_items_raw(query, max_results=30))
    ...
    # KG entity-fusion (5th source): query -> matching KG entities -> their neighbors
    kg_results = await _kg_entity_search(query, max_results=20)

    if not vector_results and not keyword_text and not item_results and not fts5_results and not kg_results:
    ...
    # Score KG entity-fusion results (query -> entities -> neighbors)
    for rank, (kid, content, score) in enumerate(kg_results):
        key = f"kg::{content[:100]}"
        rrf_scores[key] = rrf_scores.get(key, 0) + 1.0 / (k + rank + 1)
        rrf_scores[key + "::data"] = (kid, content, score)
    ...
    # Cross-encoder reranking stage. Same reboot-wedge class as the index load:
    # _get_cross_encoder() imports sentence_transformers + loads a reranker model —
    # seconds warm, unbounded if a DLL load blocks — so it must never run on the
    # loop. On timeout we serve RRF order unreranked; the load thread finishes in
    # the background and the reranker picks up on a later query.
    try:
        reranker = await asyncio.wait_for(
            loop.run_in_executor(None, _get_cross_encoder),
            timeout=getattr(config, "MEMORY_INDEX_LOAD_TIMEOUT", 60),
        )
    except asyncio.TimeoutError:
        logger.warning("Cross-encoder load timed out — serving unreranked RRF results this query.")
        reranker = None
    if reranker and len(top_candidates) > 1:
        try:
            ...
            if pairs:
                ce_scores = await loop.run_in_executor(None, lambda: reranker.predict(pairs))
...
async def _keyword_search(query: str, max_results: int = 20, raw: bool = False):
    """Async wrapper — runs the synchronous TF-IDF scan in a thread pool so a large
    corpus walk can NEVER block the event loop. Day-151 wedge fix: the sync body below
    used to run directly on the loop, holding it for minutes and silently defeating
    every cooperative wait_for timeout (the 90s build guard AND the 120s dispatch guard)."""
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, _keyword_search_sync, query, max_results, raw)


def _keyword_search_sync(query: str, max_results: int = 20, raw: bool = False):
    """TF-IDF keyword search across all memory files. Synchronous — ALWAYS via executor."""
    ...
    # Project files (skip vendored/archived/cache noise — node_modules alone was ~⅓ of recall)
    if config.PROJECTS_DIR.is_dir():
        for md_file in config.PROJECTS_DIR.glob("**/*.md"):
            if _INDEX_SKIP_PARTS.isdisjoint(md_file.parts):
                search_paths.append(md_file)

    # Skills files (skip vendored/archived/code-resource noise that pollutes recall)
    ...
    if chats_dir.is_dir():
        for html_file in chats_dir.glob("**/*.html"):
            if _INDEX_SKIP_PARTS.isdisjoint(html_file.parts):
                search_paths.append(html_file)
        for json_file in chats_dir.glob("**/*.json"):
            if _INDEX_SKIP_PARTS.isdisjoint(json_file.parts):
                search_paths.append(json_file)

    # (awesome-slash / superpowers / pragmatic-clean-code-reviewer are already covered
    # by the filtered skills glob above — no separate re-scan needed.)
