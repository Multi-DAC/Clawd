...
async def _corpus_tool(input_data: dict) -> str:
    """Async shim. Post-op A1: the entire body (chromadb + sentence-transformers
    imports, model load, encode, HNSW queries, full index passes) is synchronous
    and used to run ON the event loop — the same DLL-load wedge class fixed in
    memory_tools (4c8cd37). Runs in a worker thread under a real budget now;
    a blocked load times out instead of freezing every tool call in the process."""
    budget = 600 if input_data.get("action") == "index" else 120
    try:
        return await asyncio.wait_for(
            asyncio.to_thread(_corpus_tool_sync, input_data), timeout=budget
        )
    except asyncio.TimeoutError:
        return (f"corpus_search '{input_data.get('action', 'info')}' exceeded {budget}s "
                f"(model/DB load blocked?) — the operation continues in the background; retry shortly.")


def _corpus_tool_sync(input_data: dict) -> str:
