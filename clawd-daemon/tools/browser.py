...
async def _ensure_browser():
    global _pw, _browser
    # Post-op A6: the heavy playwright import ran ON the loop while holding
    # _init_lock — a stalled import froze the loop AND poisoned the lock for
    # every later browser call. Import in a worker thread, outside the lock.
    if _browser is None:
        def _import_playwright():
            from playwright.async_api import async_playwright
            return async_playwright
        async_playwright = await asyncio.wait_for(
            asyncio.to_thread(_import_playwright), timeout=60
        )
