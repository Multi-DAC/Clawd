...
async def _evolve_artifact(input_data: dict) -> str:
    ...
    # Post-op D3: EAC is a code-writing loop — the guardian docstring always
    # claimed it was gated; now it actually is. Artifacts stay confined to
    # CLAWD_HOME/artifacts; this adds the audit trail + protected-set block.
    if action in ("seed", "mutate"):
        from tools.guardian import guard_self_modification
        target = input_data.get("name") or input_data.get("artifact_id", "eac_artifact")
        allowed, reason = guard_self_modification(
            f"artifacts/{target}", origin="eac", details={"action": action})
        if not allowed:
            return f"[BLOCKED by guardian] {reason}"
