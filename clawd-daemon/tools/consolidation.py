...
async def consolidate_memory(router=None) -> str:
    ...
    """
    results = []
    # Post-op A4: every sync pass below walks/rewrites many memory files and used
    # to run directly ON the event loop — user messages stalled mid-consolidation.
    # Each pass now runs in a worker thread; the awaits between them stay native.

    # 1. Archive and compress old daily logs
    archived = await asyncio.to_thread(_archive_old_logs)
    if archived:
    ...
    decayed = await asyncio.to_thread(_decay_unused_items)
    if decayed:
    ...
    try:
        ...
        tiered = await asyncio.to_thread(update_item_tiers)
        if tiered:
    ...
    if router:
        try:
            ...
            recent_episodes = []
            if exp_file.exists():
                all_eps = await asyncio.to_thread(
                    lambda: json.loads(exp_file.read_text(encoding="utf-8")))
    ...
    summarized = await asyncio.to_thread(_generate_daily_summary)
    if summarized:
