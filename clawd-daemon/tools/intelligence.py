...
# Post-op 2026-07-01. Two fixes share these helpers:
# (A5) goals.json/experiences.json grow unbounded and were fully re-serialized ON
#     the event loop per call — off-loop now.
# (P0-1) the tools wrote ONLY JSON while memory_search's episode channel reads
#     SQLite FTS, refreshed only by the boot migration — so in-session goals and
#     experiences were invisible to recall until the next reboot. JSON stays
#     authoritative; every write now also mirrors to SQLite (FTS triggers keep
#     the search index live). Mirror failure is loud (WARNING) but never fatal.

async def _read_json_list(path: Path) -> list:
    if not path.exists():
        return []
    def _read():
        try:
...
async def _write_json_list(path: Path, data: list) -> None:
    def _write():
        path.write_text(json.dumps(data, indent=2, default=str), encoding="utf-8")
    await asyncio.to_thread(_write)


async def _mirror_goal(goal: dict) -> None:
    try:
        from tools.sqlite_store import upsert_goal
        await upsert_goal(goal)
    except Exception as e:
        logger.warning(f"SQLite goal mirror failed (#{goal.get('id')}): {e}")


async def _mirror_episode(episode: dict) -> None:
    try:
        from tools.sqlite_store import upsert_episode
        await upsert_episode(episode)
    except Exception as e:
        logger.warning(f"SQLite episode mirror failed (#{episode.get('id')}): {e}")


async def _delete_goal_sqlite(goal_id: int) -> None:
    try:
        ...
        logger.warning(f"SQLite goal delete failed (#{goal_id}): {e}")
...
async def _goals(input_data: dict) -> str:
    ...
    # Load existing goals (off-loop; file grows unbounded)
    goals = await _read_json_list(goals_file)
    ...
    if action == "add":
        ...
        goals.append(goal)
        await _write_json_list(goals_file, goals)
        await _mirror_goal(goal)
        ...
        await _write_json_list(goals_file, goals)
        await _delete_goal_sqlite(goal_id)
...
async def _experience(input_data: dict) -> str:
    ...
    experiences = await _read_json_list(exp_file)
    ...
    if action == "predict":
        ...
        await _write_json_list(exp_file, experiences)
        await _mirror_episode(exp)
