...
def run_health_check(input_data: dict | None = None) -> str:
    ...
    _post_tool = check_jsonl_monitor(
        "post_tool_log (Claude Code hook)",
        MEMORY_DIR / "tool_audit.jsonl",
        expected_writes_per_hour=20.0,  # active session writes ~20/hour
        severity="HIGH",
    )
    findings.append(_post_tool)
    _change_journal = check_jsonl_monitor(
        "change_journal (rollback tracker)",
        MEMORY_DIR / "change_journal.json",
        expected_writes_per_hour=2.0,  # writes happen on file mods
        severity="MEDIUM",
    )
    # Carrier-migration awareness (2026-06-17, Day 137): the change_journal records ONLY the
    # daemon's internal shell/memory tools (tools/__init__.py:178 -> _record_change_for_tool).
    # When work runs through the Claude Code harness instead (as it has since ~Jun 10), daemon-tool
    # journaling goes LEGITIMATELY quiet while the harness audit (post_tool_log / tool_audit.jsonl)
    # + git carry the audit-and-undo role. A stale change_journal is therefore EXPECTED, not a
    # failure, whenever post_tool_log is fresh. Downgrade ONLY on positive evidence the harness
    # carrier is alive — if post_tool_log is ALSO stale, the original severity stands (never blind
    # a real all-quiet stall). This is the carrier-migration pattern, not a broken writer.
    if (_change_journal.get("severity") in ("MEDIUM", "HIGH", "CRITICAL")
            and _post_tool.get("severity") == "OK"):
        _change_journal["severity"] = "LOW"
        _change_journal["status"] = "QUIET_SUPERSEDED"
        _change_journal["message"] = (
            "Quiet but expected — work is running through the Claude Code harness; audit+undo "
            "carried by post_tool_log (tool_audit.jsonl) + git. Daemon-tool journaling resumes "
            "when the daemon's own shell/memory tools are used."
        )
    findings.append(_change_journal)
