...
def _get_model(model_size: str = "large-v3", device: str = "cuda", compute_type: str = "float16"):
    ...
    if _model is not None and _model_config == cfg:
        return _model
    # New config or first call — (re)load. The offline flag is flipped only for
    # the duration of the load (post-op C-item: the old process-wide hard set
    # undid substrate's Norton-TLS offline pin for everything loaded after it).
    prev_offline = os.environ.get("HF_HUB_OFFLINE")
    os.environ["HF_HUB_OFFLINE"] = "0"  # allow first-time model fetch
    try:
...
async def _voice_input_tool(input_data: dict) -> str:
    """Async shim. Post-op A2: Whisper/ctranslate2 loads (~35s cold, unbounded if
    a DLL load blocks) and transcription ran ON the event loop — wedge class.
    Whole body now runs in a worker thread under a budget."""
    budget = 600  # transcription of long audio is legitimately slow
    try:
        return await asyncio.wait_for(
            asyncio.to_thread(_voice_input_tool_sync, input_data), timeout=budget
        )
    except asyncio.TimeoutError:
        return f"voice_input '{input_data.get('action', 'info')}' exceeded {budget}s — retry shortly."


def _voice_input_tool_sync(input_data: dict) -> str:
    global _model, _model_config
