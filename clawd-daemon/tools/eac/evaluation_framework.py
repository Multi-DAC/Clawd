...
try:
    ...
    print(f"ERROR: {{e}}")
"""
            # Real builtins: the test harness uses print/re/isinstance, and the artifact is
            # Clawd's own generated code run locally. The old `{"__builtins__": {}}` disabled
            # everything so exec always threw -> fitness was degenerate. Capture stdout to read
            # the TEST_PASSED/FAILED marker (the old code read __builtins__, never the output).
            import io as _io
...
if func_name:
    ...
    if callable(func):
        ...
        if isinstance(test_input, dict):
            ...
            func(test_input)
"""
            namespace = {}  # real builtins (see _evaluate_correctness) so setup/exec actually run
