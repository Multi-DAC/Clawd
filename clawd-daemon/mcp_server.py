...
# Windows substrate patches (Norton TLS trust, aiohttp DNS resolver, HF offline).
# This is a SEPARATE process from clawd.py — Claude Code spawns it as the MCP
# server — so it must apply the same patches (without them, clawd_speak hung 120s
# through MCP on the broken aiohttp AsyncResolver). Previously duplicated here;
# now consolidated in substrate.py. DAEMON_DIR is already on sys.path (above).
import substrate  # noqa: F401 — applies the patches on import
...
async def _call(name: str, input_data: dict) -> str:
    """Call a daemon tool handler by name — via tools.execute_tool() ONLY.

    Day 96 evening Phase 4 #1 routed dispatch through execute_tool so B1
    (safety monitor), B2 (schema validation), B9 (audit trail), and output
    compression all fire. Post-op D4 REMOVED the legacy fallback that
    dispatched raw handlers when the wrapper errored — a drift-prone second
    registry that silently bypassed every safety layer exactly when things
    were already going wrong. If the wrapper fails, the tool call fails
    loudly and the brain can route around it.
    """
    try:
        import tools as _tools
    except Exception as e:
        return f"Error: daemon tools package failed to import: {type(e).__name__}: {e}"
    if name not in _tools._TOOL_HANDLERS:
        return (f"Error: Tool '{name}' not registered. "
                f"Known: {sorted(_tools._TOOL_HANDLERS.keys())}")
    try:
...
def _preload_native_libs():
    """Load every native-DLL-heavy dependency on the MAIN thread, BEFORE the
    anyio loop starts (2026-07-02 loader-lock deadlock fix).

    The 4c8cd37 off-loop-import-with-budget fix is not survivable in this
    process: when Defender tree-flags the claude.exe spawn chain, the lazy
    numpy import parks inside LoadLibrary on an executor thread HOLDING the
    Windows loader lock — then any Thread.start() from the event-loop thread
    (executor growth for the keyword fallback, aiosqlite's per-connection
    thread) blocks forever on DLL_THREAD_ATTACH, freezing the loop so no
    wait_for/dispatch timeout can ever fire. Proven with stack dumps: the
    same imports complete in ~0.1s on this main thread while the executor-
    thread variant wedges indefinitely.

    Pre-importing here means no LoadLibrary can happen during a tool call.
    Worst case (Defender holds even this) the server start exceeds claude's
    30s connect timeout and the session runs without clawd-tools — degraded,
    not catatonic.
    """
    import tools  # registers handlers; front-loads aiosqlite/sqlite3 DLLs
    from tools import embeddings  # numpy (_multiarray_umath)
    import torch
    import sentence_transformers  # noqa: F401
    try:
        if torch.cuda.is_available():
            torch.zeros(1).to("cuda")  # force cuBLAS/cuDNN DLL loads now, not mid-search
    except Exception as e:
        logger.warning(f"CUDA warmup failed (search will fall back to CPU): {e}")
