...
# Windows substrate patches — HuggingFace offline, Norton TLS trust, and the
# aiohttp DNS-resolver fix. Consolidated in substrate.py (was copy-pasted here,
# in mcp_server.py, and in hooks/notify_telegram.py with a "keep in sync"
# warning). Must import before config / telegram / aiohttp / transformers so the
# patches apply before those libraries initialize.
...
async def run_daemon(enable_heartbeat: bool = True):
    ...
    # Start API server (Mission Control bridge) — OFF by default (unused; was
    # unauthenticated on 0.0.0.0). Enable via API_SERVER_ENABLED.
    api_server = None
    ...
    try:
        ...
        if config.DASHBOARD_ENABLED and mc_dir.exists():
            import shutil
    ...
    try:
        ...
        if avatar_pkg.exists() and getattr(config, "AVATAR_ENABLED", True):
            import shutil
    ...
    # Start A2A server (agent-to-agent protocol) — OFF by default (no peers; was
    # unauthenticated on 0.0.0.0). Enable via A2A_SERVER_ENABLED.
    a2a_runner = None
    if config.A2A_SERVER_ENABLED:
        try:
